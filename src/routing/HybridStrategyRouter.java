 ...
 * 
 * A HybridStrategyRouter maintains a List of Tuple<Message, Connection> in 
 * support of a call to ActiveRouter.tryMessagesForConnected() in 
 * HybridStrategyRouter.update(). Since update() is called so frequently, we'd 
 ...
 */
public class HybridStrategyRouter extends ActiveRouter
{
	public static final String PUBSUB_NS = "HybridStrategyRouter";
	...
	public HybridStrategyRouter(Settings s)
	{
		super(s);
	...
	public HybridStrategyRouter(HybridStrategyRouter r)
	{
		super(r);
		//outgoingMessages = new LinkedList<Tuple<Message, Connection>>();
	...
	{
		return new HybridStrategyRouter(this);
	}
	...
	public void changedConnection(Connection con)
	{
		...
		HybridStrategyRouter otherRouter = (HybridStrategyRouter)otherNode.getRouter();
		if(con.isUp())
		{
			/*Betweenness*/
	...
	private Tuple<Message, Connection> tryOtherMessages() {
		...
		for (Message m : msgCollection) {
			...
			for (Connection con : getConn()) {
				...
				HybridStrategyRouter otherRouter = (HybridStrategyRouter)other.getRouter();
				HybridStrategyRouter myRouter = (HybridStrategyRouter)getHost().getRouter();
		...
		for(Iterator<Tuple<Message, Connection>> i = ShouldSendMessages.iterator(); 
				...
				Integer isTheinactivedest = (Integer)t.getKey().getProperty("meet");
				if((nrofCopies <= 0&&isTheinactivedest == 0)||isTheinactivedest == 1){						
					i.remove();						
				}
