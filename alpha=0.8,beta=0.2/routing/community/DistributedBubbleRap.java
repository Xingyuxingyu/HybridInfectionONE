				...
				implements RoutingDecisionEngine, CommunityDetectionEngine
{
	...
	public DistributedBubbleRap(Settings s) {
		if (s.contains(COMMUNITY_ALG_SETTING))
			this.community = (CommunityDetection) s.createIntializedObject(s
					.getSetting(COMMUNITY_ALG_SETTING));
		else
			this.community = new SimpleCommunityDetection(s);

		if (s.contains(CENTRALITY_ALG_SETTING))
			this.centrality = (Centrality) s.createIntializedObject(s
					.getSetting(CENTRALITY_ALG_SETTING));
		else
	...
	public boolean shouldSendMessageToHost(Message m, DTNHost otherHost) {
		...
		Integer nrofCopies = (Integer) m.getProperty("SPRAYANDWAIT_NS" + "."
				+ "copies");
		...
		int threshold = DecisionEngineRouter.th;

		int MyLongDurationWithDest = 0, PeerLongDurationWithDest = 0;
		if (connHistory.containsKey(m.getTo())) {

			for (Duration time1 : connHistory.get(m.getTo())) {
				if ((time1.end - time1.start) > threshold)
					MyLongDurationWithDest++;
			}
		}
		if (de.connHistory.containsKey(m.getTo())) {
			for (Duration time2 : de.connHistory.get(m.getTo())) {
				if ((time2.end - time2.start) > threshold)
					PeerLongDurationWithDest++;
			}
		}
		double alpha = DecisionEngineRouter.alpha, beta = DecisionEngineRouter.beta;// cam

		if (nrofCopies >= DecisionEngineRouter.copy / 2) {
			int peermember = 0, mymember = 0 , commonmember=0;
			for (DTNHost node : de.connHistory.keySet()) {
				if (!connHistory.containsKey(node))
					peermember++;
			}
			
			for (DTNHost node : connHistory.keySet()) {
				if (!de.connHistory.containsKey(node))
					mymember++;
			}
			...
			if ((peermember + mymember != 0)) {
				double myutility = 0, peerutility = 0;
				myutility = alpha * MyLongDurationWithDest / (PeerLongDurationWithDest + MyLongDurationWithDest) 
							+ beta * mymember / (peermember + mymember + commonmember);
				peerutility = alpha * PeerLongDurationWithDest / (PeerLongDurationWithDest + MyLongDurationWithDest)
							+ beta * peermember / (peermember + mymember + commonmember);
				if ((PeerLongDurationWithDest + MyLongDurationWithDest) != 0) {
					if (peerutility > myutility)
						return true;
					else 
						return false;
				} 
				else {
					int MyLongDurationWithPeer = 0, MyShortDurationWithPeer = 0;
					if (connHistory.containsKey(otherHost)) {
						for (Duration period : connHistory.get(otherHost)) {
							if ((period.end - period.start) > threshold)
								MyLongDurationWithPeer++;
							else 
								MyShortDurationWithPeer++;
						}
						if (MyShortDurationWithPeer <= 3 && MyShortDurationWithPeer > MyLongDurationWithPeer) 
							return true;
						else 
							return false;	
					} 
				...
				return false;
		} else {// This statement will be considered when copies more than 2 (ie. 4,8,...)
			if (PeerLongDurationWithDest > MyLongDurationWithDest)
				return true;
			else
				return false;
		}
   ...
   /* try{
		String str = "false_event.txt";
		FileWriter output = new FileWriter(str, true);
		output.write(SimClock.getTime()+" "+otherHost.toString().substring(1)+
				" "+m.getTo().toString().substring(1)+"\n");
	...
	public boolean shouldDeleteSentMessage(Message m, DTNHost otherHost)
	{
		...
		return de.connHistory.keySet().contains(m.getTo())
				&&!this.connHistory.keySet().contains(m.getTo()) ;
...
public int findbubbleraprule( Message m, DTNHost from, DTNHost to) 
    {//System.out.println( from.toString() + " -- " + to.toString() );
        if(m.getTo() == to) return 1;
		
		DTNHost dest = m.getTo();
		DistributedBubbleRap de = (DistributedBubbleRap) ((DecisionEngineRouter)to.getRouter()).getDecisionEngine();
		DistributedBubbleRap my  = (DistributedBubbleRap) ((DecisionEngineRouter)from.getRouter()).getDecisionEngine();
	
		boolean peerInCommunity = de.commumesWithHost(dest);
		boolean meInCommunity = my.commumesWithHost(dest);
		...
		{ 
			...
			if(de.getLocalCentrality( ) > my.getLocalCentrality())
			{
				return 3;
			}
			else
		...
		else if(de.getGlobalCentrality() > my.getGlobalCentrality())
		{
			return 4;
		}
