...
public class densityreport extends Report implements UpdateListener {
	...
	private void printdensity(List<DTNHost> hosts) {
		
		int density = 0;
		for (DTNHost h : hosts) {
			//System.out.println(h.getConnections());
			density += h.getConnections().size();
		}
		density = density / 2;
		//System.out.println("-------------------");
		write ((int)getSimTime()/1000 + " " + density ); // simulation time stamp
	}
